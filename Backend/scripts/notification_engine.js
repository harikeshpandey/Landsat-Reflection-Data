...
// Notification stack to hold pending notifications
    ...
    try {
        ...
        obj.table = obj.table.filter(entry => {
            return !processedEntries.some(processed =>
                ...
                processed.methods === entry.methods // Add other identifiers to match as necessary
            );
        });
...
const processNotifications = async () => {
    ...
    while (notificationStack.length > 0) {
        const notification = notificationStack.shift(); // Get the next notification

        console.log("Sending notification:", notification);
        /**
         * here first we will se the methods defind to send alert to the user and then 
         * user: 'hplrn69@gmail.com',
         */
        try {
            console.log(notification)
            let to =notifications.methods[0].medium //"11.rachitpandey@gmail.com"
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f7f7f7;">
                  ...
                  <div style="padding: 20px;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin: 10px 0;">
                      ...
                      <tbody>
                        <tr>
                          <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${notifications.lat}</td>
                          <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${notifications.long}</td>
                          <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${notifications.time_stamp}</td>
                        </tr>
                      </tbody>
                    </table>
            ...
            await sendEmail(to, subject, body, process.env.EMAIL_USER, process.env.EMAIL_PASSWORD);
        ...
        await new Promise(resolve => setTimeout(resolve, 1000)); // Simulating async push notification sending
